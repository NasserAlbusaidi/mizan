...
test("--share prints safe public launch copy without reading transcripts", () => {
  ...
  assert.match(result.stdout, /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.30 -- mizan --try/);
  assert.match(result.stdout, /npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.30/);
  assert.match(
    result.stdout,
...
test("--share --output writes the public launch copy", () => {
  ...
  assert.match(markdown, /github:NasserAlbusaidi\/mizan#v0\.1\.30 -- mizan --try/);
  assert.match(markdown, /github:NasserAlbusaidi\/mizan#v0\.1\.30/);
