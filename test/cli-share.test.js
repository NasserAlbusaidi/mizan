...
test("--share prints safe public launch copy without reading transcripts", () => {
  ...
  assert.match(result.stdout, /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.53 -- mizan --try/);
  ...
  assert.match(result.stdout, /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.53 -- mizan --demo/);
  assert.match(result.stdout, /npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.53/);
  assert.match(
    result.stdout,
...
test("--share --output writes the public launch copy", () => {
  ...
  assert.match(markdown, /github:NasserAlbusaidi\/mizan#v0\.1\.53 -- mizan --try/);
  assert.match(markdown, /github:NasserAlbusaidi\/mizan#v0\.1\.53 -- mizan --weekly --demo --output "\$HOME\/Documents\/Mizan\/mizan-demo-weekly\.md"/);
  assert.match(markdown, /github:NasserAlbusaidi\/mizan#v0\.1\.53 -- mizan --demo/);
  assert.match(markdown, /github:NasserAlbusaidi\/mizan#v0\.1\.53/);
  assert.match(markdown, /releases\/download\/v0\.1\.53\/nasseralbusaidi-mizan-0\.1\.53\.tgz/);
