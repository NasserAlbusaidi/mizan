...
test("dashboard keeps demo mode visible with a copyable install-and-setup command", async () => {
  ...
  assert.match(actions, /npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.57 &amp;&amp; mizan --setup/);
  assert.match(actions, /data-copy-command="npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.57 &amp;&amp; mizan --setup"/);
  assert.match(actions, /Fallback install if GitHub tags fail/);
  assert.match(
    actions,
    /npm install -g https:\/\/github\.com\/NasserAlbusaidi\/mizan\/releases\/download\/v0\.1\.57\/nasseralbusaidi-mizan-0\.1\.57\.tgz &amp;&amp; mizan --setup/,
  );
  assert.match(
    actions,
    /data-copy-command="npm install -g https:\/\/github\.com\/NasserAlbusaidi\/mizan\/releases\/download\/v0\.1\.57\/nasseralbusaidi-mizan-0\.1\.57\.tgz &amp;&amp; mizan --setup"/,
  );
});
