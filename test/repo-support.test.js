...
test("README quick start previews value before setup", () => {
  ...
  assert.match(
    quickStart,
    /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.52 -- mizan --try/,
  );
  assert.match(
    quickStart,
    /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.52 -- mizan --weekly --demo --output "\$HOME\/Documents\/Mizan\/mizan-demo-weekly\.md"/,
  );
  assert.match(
    quickStart,
    /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.52 -- mizan --demo/,
  );
  assert.ok(
    quickStart.indexOf("npm exec --yes --package github:NasserAlbusaidi/mizan#v0.1.52") <
      quickStart.indexOf("npm install -g github:NasserAlbusaidi/mizan#v0.1.52"),
  );
  assert.ok(
    quickStart.indexOf("npm exec --yes --package github:NasserAlbusaidi/mizan#v0.1.52 -- mizan --demo") <
...
test("README documents the versioned GitHub install paths before npm publish", () => {
  ...
  assert.match(quickStart, /npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.52/);
  assert.ok(
    quickStart.indexOf("npm install -g github:NasserAlbusaidi/mizan#v0.1.52") <
      quickStart.indexOf(
...
test("launch kit gives a practical public launch script", () => {
  ...
  assert.match(kit, /npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.52/);
