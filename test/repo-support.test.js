...
test("README quick start previews value before setup", () => {
  ...
  assert.match(
    quickStart,
    /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.57 -- mizan --try/,
  );
  assert.match(
    quickStart,
    /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.57 -- mizan --weekly --demo --output "\$HOME\/Documents\/Mizan\/mizan-demo-weekly\.md"/,
  );
  assert.match(
    quickStart,
    /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.57 -- mizan --demo/,
  );
  assert.match(
    quickStart,
    /npm exec --yes --package https:\/\/github\.com\/NasserAlbusaidi\/mizan\/releases\/download\/v0\.1\.57\/nasseralbusaidi-mizan-0\.1\.57\.tgz -- mizan --try/,
  );
  assert.ok(
    quickStart.indexOf("npm exec --yes --package github:NasserAlbusaidi/mizan#v0.1.57") <
      quickStart.indexOf("npm install -g github:NasserAlbusaidi/mizan#v0.1.57"),
  );
  assert.ok(
    quickStart.indexOf(
      "npm exec --yes --package https://github.com/NasserAlbusaidi/mizan/releases/download/v0.1.57/nasseralbusaidi-mizan-0.1.57.tgz -- mizan --try",
  ...
  assert.ok(
    quickStart.indexOf("npm exec --yes --package github:NasserAlbusaidi/mizan#v0.1.57 -- mizan --demo") <
...
test("README documents the versioned GitHub install paths before npm publish", () => {
  ...
  assert.match(quickStart, /npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.57/);
  assert.ok(
    quickStart.indexOf("npm install -g github:NasserAlbusaidi/mizan#v0.1.57") <
      quickStart.indexOf(
...
test("launch kit gives a practical public launch script", () => {
  ...
  assert.match(kit, /npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.57/);
