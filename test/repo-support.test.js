...
test("README quick start previews value before setup", () => {
  ...
  assert.ok(
    quickStart.indexOf("npm exec --yes --package github:NasserAlbusaidi/mizan#v0.1.22") <
      quickStart.indexOf("npm install -g github:NasserAlbusaidi/mizan#v0.1.22"),
  );
...
test("README documents the versioned GitHub install paths before npm publish", () => {
  ...
  assert.match(quickStart, /npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.22/);
  assert.ok(
    quickStart.indexOf("github:NasserAlbusaidi/mizan#v0.1.22") <
      quickStart.indexOf("releases/download/v0.1.22"),
  );
  assert.match(
    readme,
...
test("launch kit gives a practical public launch script", () => {
  ...
  assert.match(kit, /npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.22/);
  assert.match(
    kit,
    /npm install -g https:\/\/github\.com\/NasserAlbusaidi\/mizan\/releases\/download\/v0\.1\.22\/nasseralbusaidi-mizan-0\.1\.22\.tgz/,
  );
