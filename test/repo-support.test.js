...
test("README quick start previews value before setup", () => {
  ...
  assert.match(
    quickStart,
    /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.53 -- mizan --try/,
  );
  assert.match(
    quickStart,
    /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.53 -- mizan --weekly --demo --output "\$HOME\/Documents\/Mizan\/mizan-demo-weekly\.md"/,
  );
  assert.match(
    quickStart,
    /npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.53 -- mizan --demo/,
  );
  assert.ok(
    quickStart.indexOf("npm exec --yes --package github:NasserAlbusaidi/mizan#v0.1.53") <
      quickStart.indexOf("npm install -g github:NasserAlbusaidi/mizan#v0.1.53"),
  );
  assert.ok(
    quickStart.indexOf("npm exec --yes --package github:NasserAlbusaidi/mizan#v0.1.53 -- mizan --demo") <
...
test("README documents the versioned GitHub install paths before npm publish", () => {
  ...
  assert.match(quickStart, /npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.53/);
  assert.ok(
    quickStart.indexOf("npm install -g github:NasserAlbusaidi/mizan#v0.1.53") <
      quickStart.indexOf(
        "npm install -g https://github.com/NasserAlbusaidi/mizan/releases/download/v0.1.53/nasseralbusaidi-mizan-0.1.53.tgz",
      ),
  );
  assert.match(
    readme,
...
test("launch kit gives a practical public launch script", () => {
  ...
  assert.match(kit, /npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.53/);
