...
test("--try prints a demo summary and next steps without starting the dashboard", () => {
  ...
  assert.match(result.stdout, /Install Mizan: npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.55/);
  assert.match(result.stdout, /Fallback install: npm install -g https:\/\/github\.com\/NasserAlbusaidi\/mizan\/releases\/download\/v0\.1\.55\/nasseralbusaidi-mizan-0\.1\.55\.tgz/);
  ...
  assert.match(result.stdout, /Open the sample dashboard without install: npm exec --yes --package github:NasserAlbusaidi\/mizan#v0\.1\.55 -- mizan --demo/);
  assert.match(result.stdout, /Fallback no-install demo: npm exec --yes --package https:\/\/github\.com\/NasserAlbusaidi\/mizan\/releases\/download\/v0\.1\.55\/nasseralbusaidi-mizan-0\.1\.55\.tgz -- mizan --try/);
