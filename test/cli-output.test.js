...
test("--try prints a demo summary and next steps without starting the dashboard", () => {
  ...
  assert.match(result.stdout, /Install Mizan: npm install -g github:NasserAlbusaidi\/mizan#v0\.1\.43/);
  assert.match(result.stdout, /Fallback install: npm install -g https:\/\/github\.com\/NasserAlbusaidi\/mizan\/releases\/download\/v0\.1\.43\/nasseralbusaidi-mizan-0\.1\.43\.tgz/);
