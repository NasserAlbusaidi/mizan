...
// Memo TTL sits above the dashboard's 15s poll interval so ordinary refreshes
// reuse the memo instead of re-scanning the filesystem on every tick.
...
export function compute(windowDays, { useMemo = true, demo = false, host, port } = {}) {
  ...
  // Scan enough history for previous-window comparison plus at least 30 days for
  // burn-rate and projection metrics. Bounded by mtime so a cold cache does not
  // parse the machine's entire transcript history on first load.
  ...
  const { units, cache: nextCache, stats, changed } = scan(cache, scanCutoffMs);
  // Only rewrite the cache when the scan actually parsed or evicted a file.
  // Otherwise an idle 15s poll would keep re-serializing the whole cache to disk.
  if (changed) saveCache(nextCache);
  const data = aggregate(units, displayCutoffMs, now);
